...
const rustprojname = "."
const rustlibname = "bosque"
const juliapackage = "Bosque"
...
function build_dylib()
    ...
    #run(Cmd(`RUSTFLAGS="-C target-cpu=native" cargo build --release`, dir=joinpath(@__DIR__, rustprojname)))
...
const $libname = joinpath(@__DIR__, "..", "target", "release", "$libfile")
...
"""
    ...
    open(joinpath(@__DIR__, "examples", "deps.jl"), "w") do f
        write(f, script)
    end
end
